---
title: Blacksmithing Done Right
description: ''
tags: []
systems:
  - 5e
renderer: V3
theme: 5ePHB
snippets:
  - name: brew_snippets
    subsnippets:
      - name: example snippet
        gen: >-

          The text between `\snippet title` lines will become a snippet of name
          `title` as this example provides.


          This snippet is accessible in the brew tab, and will be inherited if
          the brew is used as a theme.

---

{{frontCover}}

{{logo ![](/assets/naturalCritLogoRed.svg)}}

# Blacksmithing Done Right
## Silvercrow605
___

{{banner HOMEBREW}}

{{footnote
  Not all weapons are the same. including the materials they're made of.
}}

![background image](https://i.imgur.com/BSDgc00.jpeg){position:absolute,bottom:0,left:0,height:100%}

{{artist,top:300px,right:3px
## Art By:
#### [Radomir Markovic](https://www.behance.net/radomarkovic)
}}


\page


### Crafting
Making a weapons is a step-by-step process. The individual parts are first shaped or forged. When every component has been built, you select the weapon you want to build and put the pieces together to create the end result.

When crafting, you must make an initial check against the DC. If the process takes longer than 2 hours, you make an additional check for every 2 hours of work after the first.
:
The special conditions of the material only need to be met for the first 15 minutes of each 2-hour block. Once those 15 minutes are met, the weapon can be worked on as normal. This rule exists to make crafting possible during adventures, and to help with very hard or hell level materials, where the special condition isn’t permanent or easy to maintain.

Once all the parts are ready, you can assemble the weapon easily over the course of 3 hours.
### Crafting Time
The amount of time it takes to make a part depends on how much material is being used, and the number given by the material. For example, making a iron large blade, would take 1 x 8 hours, equaling 8 hours. Always round up to the nearest .25, .5, .75 or whole number. These equal 15 minutes, 30 minutes, 45 minutes, and an hour. The lowest amount of time that a part can take is 15 minutes.
### Crafting Mishaps

If you fail to make the check, a complication happens, resulting in a flaw in the part. Roll on the table below to determine the flaw. If you fail 3 times on a part, it becomes scrap. Finished parts can be used to make weapons that require said parts.



#### Handle  
| D6  |     Flaw      | Description                                                                                                                                            |
| :-- | :-----------: | :----------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | Slippery Grip | Handle becomes slick when wet, imposing disadvantage on attack rolls in rain or when bloodied.                                                         |
| 2   |  Splintered   | Deals 1 damage to the wielder every time they roll a natural 1 on an attack.                                                                           |
| 3   | Poor Balance  | Weapon’s weight distribution is off, reducing damage by 1 or lowering attack speed.                                                                    |
| 4   |   Loose Fit   | Handle creaks and shifts in the wielder’s hands, giving disadvantage on precision-based attacks.                                                       |
| 5   | Warped Shape  | Causes mild wrist strain; after prolonged use, the wielder takes 1 level of exhaustion every 20 attacks with this weapon until they take a short rest. |
| 6   | Fragile Grip  | Handle has a low break threshold; on a critical failure, the handle snaps and the weapon becomes unusable till replaced.                               |


\column

#### Binding  
| D6  |        Flaw        | Description                                                                                                                                                                                                                                           |
| :-- | :----------------: | :---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   |   Loose Lashings   | The weapon’s parts rattle, producing noise and giving disadvantage on stealth.                                                                                                                                                                        |
| 2   |  Fraying Material  | Every time the weapon is used, roll a d20; on a 1, the binding breaks, and it is unusable till replaced.                                                                                                                                              |
| 3   |     Stiff Bind     | Reduces swing recovery speed, making the weapon slower to ready. Drawing or swapping to this weapon costs a bonus action instead of a free action.                                                                                                    |
| 4   |    Twist Shift     | The binding has uneven tension, causing the weapon’s parts to subtly rotate after repeated swings. Roll 2d20, if both numbers match, you must make a DC 10 Dexterity saving throw, or suffer disadvantage on the next attack as you adjust your grip. |
| 5   | Uncomfortable Bind | The seam digs into the wielder’s hands, causing 1 damage per hour of use.                                                                                                                                                                             |
| 6   |   Loose Strands    | The binding’s fibres slowly unravel with use, causing small bits of debris or dust to fall off. This leaves a faint trail behind the wielder that can be tracked easily for 1 hour after combat.                                                      |


#### Head  
| D6  |       Flaw       | Description                                                                                                                                                                                   |
| :-- | :--------------: | :-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   |  Poor Tempering  | On a natural 1, the head cracks and becomes unusable till replaced.                                                                                                                           |
| 2   |       Bent       | the head is bent, making it easier to mess up an attack. You can score a critical fail on a 1 or a 2.                                                                                         |
| 3   | Unstable Weight  | Throws off swing accuracy; ranged or thrown weapons gain disadvantage.                                                                                                                        |
| 4   | Brittle Material | Deals normal damage, but any damage roll of max value causes the head to shatter.                                                                                                             |
| 5   |   Uneven head    | The weapon head’s uneven surface causes it to pull slightly to one side during swings, making it harder to control. You have disadvantage on opportunity attacks.                             |
| 6   |  Vibrating Edge  | The weapon head produces a subtle vibration on impact. Roll 2d20, if both numbers match, you must succeed on a DC 12 Constitution saving throw or have disadvantage on your next attack roll. |

{{descriptive
##### Less Downtime Optional Rule
If your game doesnt have a large amount of downtime, the DM could choose to rule that working on a part can be done over the course of long rests, doing 2 hours per rest, or taking up to 8, but gainning a point of exaustion.
}}


\page

{{descriptive
##### Artificer Optional Rule
Any features from artificer or similar features count towards crafting parts. Such as *Magic Item Adept* would grant quater normal time and half cost to common and uncommon parts.
}}
### Material Types
| Material Type | Description |
|:--------------|:------------|
| **Metal**     | Refined from ores such as iron, steel, or mithral. Durable and forgeable. |
| **Stone**     | Mineral-based, often crystalline or volcanic like obsidian or voidglass. |
| **Organic**   | Organic materials such as wood, coral, or bone. Most beast or plant creatures apply to this category. |
| **Monstrous** | Harvested from magical or monstrous creatures—like dragonhide, chitin, or infernal horn. |
| **Planar**    | Sourced from other planes of existence. Includes rare and powerful materials like hellsteel or celestium. |
| **Alchemical**| Synthesized, magically-infused, or chemically altered substances. Often volatile or unstable. |


### Tools Required
| Tool | Description |
|:----|:------------|
| **Alchemists supplies**|	Lets you turn substances into weapons.
| **Calligraphers supplies**|	Lets you craft with paper, ink, magic, and runes.
| **Glassblowers tools**|	Lets you shape glass-like materials.
| **Jewelers tools**|	Lets you cut gems into parts.
| **Masons tools**|	Lets you carve stone into weapon parts.
| **Smiths tools**|	Lets you forge parts from metal.
| **Tinkers tools**|	Lets you make mechanical parts.
| **Weavers tools**|	Lets you braid and weave fibers into parts.
| **Woodcarvers tools**|	Lets you carve wood, bone, or similar materials.
| **Cooks Utensils**| Lets you bake your own parts.
| **Special**|	Some materials need more than tools—like forging in extreme heat, freezing cold, or even inside another dimension.

### Material Harvesting
To gain materials, you have to harvest them or purchase them. The methods of harvesting vary depending on the source, but unless otherwise stated, you receive 1d4 materials when harvesting from metal ores, stones, or wood-like materials.


\column

### Weapon Parts
Each weapon uses different parts, and each part needs different amounts of materials.

| Part | Materials Used |
|:----|:------------|
|Tool Rod| 0.5|
|Wide Guard| 0.5|
|Hand Guard| 0.5|
|Crossbar| 0.5|
|Binding| 0.5
|Knife Blade| 0.5
|Axe Head| 1
|Plate| 1
|Sword Blade| 1
|Arrowhead| 1 (per set of arrows)
|Tough Rod| 3
|Tough Binding| 3
|String | 3 (Flexible Only)
|Large Plate| 4
|Large Axe Head| 4
|Large Blade| 8


### Part Properties
Each weapon is made up of multiple parts, typically including a Head, Handle, and Binding (some weapons may add or remove components depending on their type). Each part is crafted from a material, and the material grants properties to the finished weapon.

These properties represent mechanical traits—such as attack bonuses, utility effects, or penalties that are applied to the finished weapon based on the materials used.

#### Property Stacking

If the same property appears on more than one part of the weapon, its effect increases. This means that choosing a material with a strong property for multiple parts will increase its power for the final item. For example, if both the Head and Handle grant the Magnetic property, then the weapon receives a double effect from Magnetic.

The exact mechanical stacking (e.g. additive, multiplicative, increasing intensity) depends on the property itself and is detailed in each property’s definition.

#### Parts By Color
{{column-count:2

| Color       | Part      |
| :---------- | :-------- |
| Red         | Handle    |
| Dark Red    | Handle 2  |
| Blue        | Head      |
| Dark Blue   | Head 2    |
| Orange      | Binding   |
| Dark Orange | Binding 2 |
}}
***This has no mechanical difference, just appearances.***
***Note: Images will be added once parts are finalized.***
\page

| Property | Description |
|----------|-------------|
| Acidic | Once per turn, when you hit a creature with this weapon, it takes an additional 1 acid damage. This increases by 1 with every level of Acidic.  
| Blunt | The weapon deals bludgeoning damage regardless of base type.  
| Breakable | The weapon breaks over time. After 5 rounds, the part will shatter, and the weapon won't be usable till replaced. For each additional level of Breakable, the number of rounds will go down by 1, to a minimum of 1.  
| Burning | Once per turn, when you hit a creature with this weapon, it takes an additional 1 fire damage. This increases by 1 with every level of Burning.  
| Cheapskate | On a natural 1 attack roll, the weapon loses any bonuses until the end of your next turn.  
| Chilling | Once per turn, when you hit a creature with this weapon, it takes an additional 1 cold damage. This increases by 1 with every level of Chilling.  
| Corrosive | On a hit, the weapon deals 1 acid damage once per turn. This increases by 1 with every level of corrosive.  
| Dangerous | The weapon deals an additional damage die per level of dangerous. This only applies to the first attack with the weapon in combat.  
| Draconic Will | When you fail a saving throw, you may reroll it and must use the new result. Once used, this can't be used again until you finish a short or long rest. Every level of Draconic Will gives an additional use.  
| Empowed Core | Weapon deals an extra 1 damage of the source's elemental type. This increases by 1 with every level of Empowered Core.  
| Endothermic | The weapon shifts depending on whether it deals more fire or cold damage: if fire damage is dominant, the part liquifies and gains +5 feet of reach, while if cold damage is dominant, the weapon hardens and deals an additional 2 bludgeoning damage. Each level of Endothermic increases the reach bonus by 5 feet when fire-dominant or the bludgeoning damage bonus by 2 when cold-dominant.
| Engulf | As a bonus action, the weapon engulfs the spot hit, grappling the target. The target must make a strength saving throw against your strength (athletics) check. While engulfed, the weapon cannot be used. Each additional level of Engulf grants a +1 bonus to your check.  
| Flexible | Can be used to make whips or chain weapons.  
| Glowing | The weapon glows 10 ft of bright light, and 10 ft of dim light while unsheathed. This increases by 10 with every level of Glowing.  
| Heavy | Increases the weapon’s weight, giving it the Heavy property if it didn’t already have it. If you get this twice, as well as the lightweight property, one cancels it out, the other stays. 


| | |
|----------|-------------|
| Heat Retention | As a reaction when the wielder takes fire damage, you take half the damage, and gain an additional 1d6 fire damage to your next attack. This can be done a number of times equal to the weapons level in Heat Retention per long rest, as well as increasing the damage by 1d6 per level.  
| Hollow | This item has a cavity that can store a single Tiny object. Retrieving or stashing the item requires an action. A poison or potion can be stored in it as well, and can be broken as part of an attack. 
| Holy | Once per turn, when you hit a creature with this weapon, it takes an additional 1 radiant damage. This increases by 1 with every level of Holy. 
| Insanity | The weapon is constantly whispering to you. Whenever you are hit, you must make a wisdom saving throw (DC: 11 + Insanity level) on a fail, you must roll on the short-term madness table. If the Insanity level reaches 4, and you fail by 5 points or more, you roll on the long-term madness table. If this is on a sentient weapon, the weapon is always Evil.
| Jagged Edge | The weapon gives a bleeding effect for 1 round, causing 1 slashing damage at the beginning of the creature's turn. The damage and duration increase by 1 per level of Jagged Edge.  
| Keen | Attacks made with this weapon score a critical hit on a roll of 19 or 20.  
| Leeching | Upon hitting a target, you can regain half of the weapon's damage type as HP. This means that if the weapon does piercing damage, for example, only the piercing damage dealt will be given as health. This can be done a number of times equal to the weapons level in leeching, and recharges on a long rest. 
| Lightweight | Weapons lose the Heavy property, or gain the light property if the weapon doesn't have the heavy property. This property doesn't stack, but if you get it twice, as well as the heavy property, one cancels it out, the other stays.
| Lightning rod | As a reaction when the wielder takes lightning damage, you take half the damage, and gain an additional 1d6 lightning damage to your next attack. This can be done a number of times equal to the weapons level in Lightning Rod per long rest, as well as increasing the damage by 1d6 per level.  
| Loud | Once per turn, when you hit a creature with this weapon, it takes an additional 1 thunder damage. This increases by 1 with every level of Loud.  
| Magic Focus | The weapon can be used as a spellcasting focus.  
| Magic Resistant | You have advantage on saving throws against spells while holding this weapon.  
| Magical | This weapon counts as magical for the purpose of overcoming resistance and immunity to nonmagical attacks and damage.  





\page

| | |
|----------|-------------|
| Magnetic | Can attract or repel metal objects and weapons within 5ft (DM's discretion). This increases by 5ft with every level of Magnetic. 
| Moon-touched | This weapon emits dim light in a 15-foot radius. While in moonlight or total darkness, you gain a +1 bonus to attack rolls made with it. This increases by 1 with every level of Moon-touched.  
| Nonlethal | The weapon can never kill. Always knocking the target unconscious instead.  
| Planar | This weapon deals an extra 1 damage to creatures not on their native plane. This increases by 1 with every level of Planar.  
| Poisonous | Once per turn, when you hit a creature with this weapon, it takes an additional 1 poison damage. This increases by 1 with every level of Poisonous.  
| Polished | Gains a +1 bonus to attack rolls when wielded with two hands. Each level of Polished increases this bonus by 1.  
| Reinforced | When making attacks, you can reroll a damage roll of 1 on a weapon attack with it once per round. Each additional level of Reinforced lets you reroll an additional time per round.  
| Returning | When thrown, the weapon returns to the wielder's hand immediately after the attack.  
| Sentient | The weapon has a conscience of its own. Roll a 4d6 for its intelligence, wisdom, and charisma scores, dropping the lowest roll and totalling the rest. Roll a d100 to determine its alignment, unless the material gives one; if multiple are given, you choose one of them. **Alignments:** 01-15 Lawful good, 16-35 Neutral good, 36-50 Chaotic good, 51-63 Lawful neutral, 64-73 Neutral, 74-85 Chaotic neutral, 86-89 Lawful evil, 90-96 Neutral evil, 97-00 Chaotic evil. A sentient item has some ability to communicate, with its form depending on the level of sentient the weapon has. **Communication:** level 1: The item communicates by transmitting emotion to the creature carrying or wielding it. Level 2: The item can speak, read, and understand one language. Level 3 and up: The item can speak, read, and understand 2 languages. In addition, the item can communicate telepathically with any character that carries or wields it. A sentient item can perceive its surroundings out to a limited range, also determined by its sentient level. **Senses:** level 1: Hearing and normal vision out to 30 feet. Level 2: Hearing and normal vision out to 60 feet. Level 3: Hearing and normal vision out to 120 feet. Level 4 and up: Hearing and darkvision out to 120 feet. The DM may determine other things, such as if it has a special purpose, or if something will develop further from it in time. ***Warning: Sentient over level 6 is able to control the weapon itself, being able to supress positives, and add negatives to its own accord, and attempt to take over the wielder (DC 10 + Int Mod + level.) be sure to stay on its good side, and it can assist you instead of harm.***
| Shattered | Upon getting a critical fail with this weapon twice in a row, the part shatters, making the weapon unusable until replaced.  


| | |
|----------|-------------|
| Sharpened | The weapon deals slashing damage regardless of base type.  
| Shimmering | Grants an additional +1 bonus to armor class. This increases by 1 with every level of Shimmering.  
| Shocking | Once per turn, when you hit a creature with this weapon, it takes an additional 1 lightning damage. This increases by 1 with every level of Shocking.  
| Siege | The weapon deals double damage to objects and structures.
| Slippery Grip | Handle becomes slick when wet, imposing disadvantage on attack rolls in rain or when bloodied.  
| Soft | The weapon does no base weapon damage when attacking. Any additional damage or effects are still applied.  
| Spiked | The weapon deals Piercing damage regardless of base type.  
| Sterile | While holding, you have advantage on saving throws to resist diseases.
|Sticky| You gain advantage on checks to keep hold of the weapon.
| Wild Negative | You gain a random negative property rolled from this list using a D6: 1 - Cheapskate, 2 - Soft, 3 - Slippery Grip, 4 - Breakable, 5 - Shattered, 6 - Nonlethal. Additional levels of Wild Negative count as the new properties level. A part can have Wild Negative multiple times, but will not go to the next level till all Wild Negative properties are the same level. As an example, if you have Wild Negative 2 and Wild Negative 1, the next level up goes to Wild Negative 1.  
| Wild Positive | You gain a random positive property rolled from this list using a D12: 1 - Keen, 2 - Magnetic, 3 - Corrosive, 4 - Hollow, 5 - Chilling, 6 - Shocking, 7 - Burning, 8 - Poisonous, 9 - Lightning rod, 10 - Heat Retention, 11 - Heavy, 12 - Lightweight. Additional levels of Wild Positive count as the new properties level. A part can have Wild Positive multiple times, but will not go to the next level till all Wild Positive properties are the same level. As an example, if you have Wild Positive 2 and Wild Positive 1, the next level up goes to Wild Positive 1.  
| Withering | Once per turn, when you hit a creature with this weapon, it takes an additional 1 necrotic damage. This increases by 1 with every level of Withering.  
|Longer | The weapon gains 5ft of reach per level of Longer.
|Shorter | Removes 5ft of reach, per level of Shorter to a minimum of 5ft.

\page
## Weapon Recipes

### Arm Wraps 
*These apply the properties to your unarmed attacks.*
#### Weapon Parts [Total: 3]
|  |
|:----------|
| String [Binding]
:::
### Club

#### Weapon Parts [Total: 0.5]
|  |
|:----------|
| Tool Rod [Head]
:::
### Dagger

#### Weapon Parts [Total: 1]
|  |
|:----------|
| Tool Rod [Handle]
| Knife Blade [Head]
::
### Gauntlets
*These apply the properties to your unarmed attacks.*
#### Weapon Parts [Total: 6]
|  |
|:----------|
| Large Plate [Head]
| Plate [Binding]
| Plate [Binding 2]
:

### Greatclub

#### Weapon Parts [Total: 3]
|  |
|:----------|
| Tough Rod [Head]
:::
### Handaxe

#### Weapon Parts [Total: 1.5]
|  |
|:----------|
| Tool Rod [Handle]
| Binding [Binding]
| Axe Head [Head]
:
### Javelin

#### Weapon Parts [Total: 1.5]
|  |
|:----------|
| Tool Rod [Handle]
| Tool Rod [Handle 2]
| Knife Blade [Head]
:
\page
### Light hammer

#### Weapon Parts [Total: 1.5]
|  |
|:----------|
| Tool Rod [Handle]
| Plate [Head]
::

### Mace

#### Weapon Parts [Total: 1.5]
|  |
|:----------|
| Tool Rod [Handle]
| Plate [Head]
::
### Quarterstaff

#### Weapon Parts [Total: 1]
|  |
|:----------|
| Tool Rod [Head]
| Tool Rod [Handle]
::

### Sickle

#### Weapon Parts [Total: 1]
|  |
|:----------|
| Tool Rod [Handle]
| Knife Blade [Head]
::
### Spear

#### Weapon Parts [Total: 1.5]
|  |
|:----------|
| Tool Rod [Handle]
| Tool Rod [Handle 2]
| Knife Blade [Head]
:
### Light Crossbow

#### Weapon Parts [Total: 4]
|  |
|:----------|
| Tool Rod [Head]
| Tool Rod [Handle]
| String [Binding]
:
### Dart

#### Weapon Parts [Total: 0.5]
|  |
|:----------|
| Tool Rod [Head]
:::
### Shortbow

#### Weapon Parts [Total: 4]
|  |
|:----------|
| Tool Rod [Head]
| Tool Rod [Head 2]
| String [Binding]
:
\page

### Sling

#### Weapon Parts [Total: 1]
|  |
|:----------|
| Plate (Flexible Only) [Head]
:::

### Bolts & Arrows (20)

#### Weapon Parts [Total: 1.5]
|  |
|:----------|
| Tool Rod [Handle]
| Arrow Head [Head]
::
### Battleaxe

#### Weapon Parts [Total: 5]
|  |
|:----------|
| Axe Head [Head]
| Axe Head [Head 2]
| Tough Rod [Handle]
:
### Flail

#### Weapon Parts [Total: 4.5]
|  |
|:----------|
| Plate [Head]
| String [Binding]
| Tool Rod [Handle]
:
### Glaive

#### Weapon Parts [Total: 7]
|  |
|:----------|
| Tough Rod [Handle]
| Tough Rod [Handle 2]
| Sword Blade [Head]
:
### Greataxe

#### Weapon Parts [Total: 11]
|  |
|:----------|
| Tough Rod [Handle]
| Tough Binding [Binding]
| Large Axe Head [Head]
| Axe Head [Head 2]

### Greatsword

#### Weapon Parts [Total: 14]
|  |
|:----------|
| Tough Rod [Handle]
| Tough Rod  [Binding]
| Large Blade [Head]
:
### Halberd

#### Weapon Parts [Total: 5]
|  |
|:----------|
| Tough Rod [Handle]
| Sword Blade [Head]
| Axe Head [Head 2]
:

\page
### Lance

#### Weapon Parts [Total: 14]
|  |
|:----------|
| Tough Rod [Handle]
| Tough Binding [Binding]
| Large Blade [Head]
:

### Longsword

#### Weapon Parts [Total: 2]
|  |
|:----------|
| Sword Blade [Head]
| Wide Guard [Binding]
| Tool Rod [Handle]
:
### Maul

#### Weapon Parts [Total: 8]
|  |
|:----------|
| Tough Rod [Handle]
| Tough Binding [Binding]
| Plate [Head]
| Plate [Head 2]

### Morningstar

#### Weapon Parts [Total: 7]
|  |
|:----------|
| Tough Rod [Handle]
| Tough Binding [Binding]
| Plate [Head]
:
### Pike

#### Weapon Parts [Total: 3]
|  |
|:----------|
| Tool Rod [Handle]
| Tool Rod [Handle 2]
| Axe Head [Head]
| Sword Blade [Head 2]

### Rapier

#### Weapon Parts [Total: 2]
|  |
|:----------|
| Sword Blade [Head]
| Crossbar [Binding]
| Tool Rod [Handle]
:
### Scimitar

#### Weapon Parts [Total: 2]
|  |
|:----------|
| Sword Blade [Head]
| Hand Guard [Binding]
| Tool Rod [Handle]
:
### Shortsword

#### Weapon Parts [Total: 2]
|  |
|:----------|
| Sword Blade [Head]
| Wide Guard [Binding]
| Tool Rod [Handle]
:
\page
### Trident

#### Weapon Parts [Total: 2.5]
|  |
|:----------|
| Knife Blade [Head]
| Sword Blade [Head 2]
| Binding [Binding]
| Tool Rod [Handle]

### War pick

#### Weapon Parts [Total: 5]
|  |
|:----------|
| Tough Rod [Handle]
| Binding [Binding]
| Knife Blade [Head]
| Plate [Head]

### Warhammer

#### Weapon Parts [Total: 5.5]
|  |
|:----------|
| Tool Rod [Handle]
| Tough Binding [Binding]
| Plate [Head]
| Plate [Head]

### Whip 

#### Weapon Parts [Total: 3.5]
|  |
|:----------|
| String [Head]
| Tool Rod [Handle]
::
### Blowgun 

#### Weapon Parts [Total: 0.5]
|  |
|:----------|
| Tool Rod [Head]
:::
### Hand Crossbow

#### Weapon Parts [Total: 6]
|  |
|:----------|
| Tough Rod [Head]
| String [Binding]
::
### Heavy Crossbow

#### Weapon Parts [Total: 9]
|  |
|:----------|
| Tough Rod [Head]
| Tough Rod [Handle]
| String [Binding]
:
### Longbow

#### Weapon Parts [Total: 9]
|  |
|:----------|
| Tough Rod [Head]
| Tough Rod [Head 2]
| String [Binding]
:

\page


## Common Materials

### Air
Its just solid air. Did you think it would be useful?
:
**Material Type:** Alchemical   
**Rarity:** Common  
**Tools:** [Alchemists]    
**Difficulty:** DC: 11  
**Crafting Time:** 1 x Cost Hours

{{column-count:3

#### Handle  
| Property |
|:----------|
| Lightweight
| Flexible
| Soft
| Breakable 5

#### Binding  
| Property |
|:----------|
| Lightweight
| Flexible
| Soft
| Breakable 5

#### Head  
| Property |
|:----------|
| Lightweight
| Flexible
| Soft
| Breakable 5

}}

### Bamboo

:
**Material Type:** Organic  
**Rarity:** Common  
**Tools:** [Woodcarvers]    
**Difficulty:** DC: 11  
**Crafting Time:** 1 x Cost Hours

{{column-count:3

#### Handle  
| Property |
|:----------|
| Lightweight 
| Slippery Grip

#### Binding  
| Property |
|:----------|
| Flexible
| Cheapskate

#### Head  
| Property |
|:----------|
| Blunt
| Lightweight

}}

### Bone

:
**Material Type:** Organic  
**Rarity:** Common  
**Tools:** [Woodcarvers]    
**Difficulty:** DC: 10  
**Crafting Time:** 0.75 x Cost Hours

{{column-count:3

#### Handle  
| Property |
|:----------|
| Hollow

#### Binding  
| Property |
|:----------|
| ---

#### Head  
| Property |
|:----------|
| Spiked

}}


\column

### Bread
Its bread. Why? Why not. Battle Baguette!
:
**Material Type:** Organic  
**Rarity:** Common  
**Tools:** [Cooks]  
**Difficulty:** DC: 9  
**Crafting Time:** 0.5 x Cost Hours

{{column-count:3

#### Handle  
| Property |
|:----------|
| Lightweight	|
| Soft      |
| Cheapskate | 

#### Binding  
| Property |
|:----------|
| Lightweight	|
| Soft      |
| Cheapskate | 


#### Head  
| Properties
|:----------|
| Lightweight	|
| Soft      |
| Cheapskate | 

}}

### Copper  

:
**Material Type:** Metal  
**Rarity:** Common  
**Tools:** [Smiths]    
**Difficulty:** DC: 11  
**Crafting Time:** 1 x Cost Hours

{{column-count:3

#### Handle  
| Property |
|:----------|
| Sterile

#### Binding  
| Property |
|:----------|
| Flexible

#### Head  
| Property |
|:----------|
| Lightning rod

}}


### Glass  

:
**Material Type:** Stone  
**Rarity:** Common  
**Tools:** [Glassblowers]    
**Difficulty:** DC: 11  
**Crafting Time:** 1 x Cost Hours

{{column-count:3

#### Handle  
| Property |
|:----------|
| Breakable 5
| Jagged Edge
| Dangerous

#### Binding  
| Property |
|:----------|
| Breakable 5
| Jagged Edge
| Dangerous

#### Head  
| Property |
|:----------|
| Breakable 5
| Jagged Edge
| Dangerous
| Sharpened


}}



### Iron  
Iron is the default material used in creating weapons and armor. It is solid and reliable, but offers no special traits or enhancements.  
:
**Material Type:** Metal  
**Rarity:** Common  
**Tools:** [Smiths]  
**Difficulty:** DC: 11  
**Crafting Time:** 1 x Cost Hours

{{column-count:3

#### Handle  
| Property |
|:----------|
| ---

#### Binding  
| Property |
|:----------|
| ---


#### Head  
| Properties
|:----------|
| ---

}}

\page

### Lead  

:
**Material Type:** Metal  
**Rarity:** Common  
**Tools:** [Smiths]    
**Difficulty:** DC: 12  
**Crafting Time:** 1.25 x Cost Hours

{{column-count:3

#### Handle  
| Property |
|:----------|
| Heavy

#### Binding  
| Property |
|:----------|
| Heavy

#### Head  
| Property |
|:----------|
| Heavy
| Poisonous

}}


### Scrap

Scrap is gainned whenever you fail to craft a part, or is destroyed due to a property, or external source. You gain half the materials used in the original part as scrap, if the part costs less than 1 to craft, no scrap is gainned.
:
Its so mixed with everything else in the pile of scrap, you have no idea what your gonna get.
:
**Material Type:** Roll a D8: 1-2 Metal, 3-4 Stone, 5-6 Organic, 7-8 Monstrous (determined when crafting)  
**Rarity:** Common  
**Tools:** [Smiths - metal, Woodcarvers - organic and monstrous, Masons - stone]    
**Difficulty:** DC: 11  
**Crafting Time:** 1 x Cost Hours

{{column-count:3

#### Handle  
| Property |
|:----------|
| Wild Positive
| Wild Negative

#### Binding  
| Property |
|:----------|
| Wild Positive
| Wild Negative

#### Head  
| Property |
|:----------|
| Wild Positive
| Wild Positive
| Wild Negative

}}



### Stone

:
**Material Type:** Stone  
**Rarity:** Common  
**Tools:** [Masons]    
**Difficulty:** DC: 11  
**Crafting Time:** 1 x Cost Hours

{{column-count:3

#### Handle  
| Property |
|:----------|
| Heavy

#### Binding  
| Property |
|:----------|
| ---

#### Head  
| Property |
|:----------|
| Blunt

}}


\column

### Tin  

:
**Material Type:** Metal  
**Rarity:** Common  
**Tools:** [Smiths]    
**Difficulty:** DC: 11  
**Crafting Time:** 1 x Cost Hours

{{column-count:3

#### Handle  
| Property |
|:----------|
| Lightweight

#### Binding  
| Property |
|:----------|
| Lightweight

#### Head  
| Property |
|:----------|
| Soft

}}




### Weak Construct

The part itself is a construct. Granted its the weakest form there is, just being alive. This can be made by making a golem/construct in the shape of the part, or by carving/reforging an existing construct thats CR 1 or lower.
:
If carved or reforged, it retains its mental stats and alignment. If crafted, roll its stats, and you choose its alignment.
:
**Material Type:** Monstrous OR Alchemical  
**Rarity:** Common  
**Tools:** [Smiths/Masons/Woodcarvers (When crafted using an existing construct) OR Tinkers (Crafting)]    
**Difficulty:** DC: 12  
**Crafting Time:** 1.25 x Cost Hours

{{column-count:3

#### Handle  
| Property |
|:----------|
| Sentient

#### Binding  
| Property |
|:----------|
| Sentient

#### Head  
| Property |
|:----------|
| Sentient

}}


### Wood
Wood is the most common material you can make any parts out of. Wooden parts are mediocre, but easy to come by.
:
**Material Type:** Organic  
**Rarity:** Common  
**Tools:** [Woodcarvers]  
**Difficulty:** DC: 9  
**Crafting Time:** 0.5 x Cost Hours

{{column-count:3

#### Handle  
| Property |
|:----------|
| Lightweight |

#### Binding  
| Property |
|:----------|
| Cheapskate |


#### Head  
| Properties
|:----------|
| Blunt        |
| Cheapskate | 

}}


\page
## Uncommon Materials

### Brass  

:
**Material Type:** Metal  
**Rarity:** Uncommon  
**Tools:** [Smiths]   
**Difficulty:** DC: 13  
**Crafting Time:** 2.25 x Cost Hours

{{column-count:3

#### Handle  
| Property |
|:----------|
| Sterile

#### Binding  
| Property |
|:----------|
| Heat Retention

#### Head  
| Property |
|:----------|
| Heat Retention
| Cheapskate

}}


### Bronze  

:
**Material Type:** Metal  
**Rarity:** Uncommon  
**Tools:** [Smiths]  
**Difficulty:** DC: 13  
**Crafting Time:** 2.25 x Cost Hours

{{column-count:3

#### Handle  
| Property |
|:----------|
| Sterile

#### Binding  
| Property |
|:----------|
| Reinforced

#### Head  
| Property |
|:----------|
| Polished

}}


### Cold Iron  

:
**Material Type:** Metal  
**Rarity:** Uncommon  
**Tools:** [Smiths, Special (Cold)]    
**Difficulty:** DC: 12  
**Crafting Time:** 2 x Cost Hours

{{column-count:3

#### Handle  
| Property |
|:----------|
| ---

#### Binding  
| Property |
|:----------|
| ---

#### Head  
| Property |
|:----------|
| Chilling
| Magical

}}


\column

### Construct

The part itself is a construct. Slightly stronger than its former tier, being more durable overall. This can be made by making a golem/construct in the shape of the part, or by carving/reforging an existing construct thats CR 2--4.
:
If carved or reforged, it retains its mental stats and alignment. If crafted, roll its stats, and you choose its alignment.
:
**Material Type:** Monstrous OR Alchemical  
**Rarity:** Uncommon  
**Tools:** [Smiths/Masons/Woodcarvers (When crafted using an existing construct) OR Tinkers (Crafting)]    
**Difficulty:** DC: 13  
**Crafting Time:** 2.25 x Cost Hours

{{column-count:3

#### Handle  
| Property |
|:----------|
| Sentient
| Reinforced

#### Binding  
| Property |
|:----------|
| Sentient
| Reinforced

#### Head  
| Property |
|:----------|
| Sentient
| Reinforced

}}

### Frostwood

:
**Material Type:** Organic  
**Rarity:** Uncommon  
**Tools:** [Woodcarvers, Special (Cold)]   
**Difficulty:** DC: 12  
**Crafting Time:** 2 x Cost Hours

{{column-count:3

#### Handle  
| Property |
|:----------|
| Lightweight
| Magic Focus
| Slippery Grip

#### Binding  
| Property |
|:----------|
| Chilling
| Flexible

#### Head  
| Property |
|:----------|
| Chilling
| Magical
| Shattered

}}

### Gallium  

:
**Material Type:** Metal  
**Rarity:** Uncommon  
**Tools:** [Calligraphers (have it return to its original shape after melting)]   
**Difficulty:** DC: 13  
**Crafting Time:** 2.25 x Cost Hours

{{column-count:3

#### Handle  
| Property |
|:----------|
| Endothermic
| Flexible
| Slippery Grip

#### Binding  
| Property |
|:----------|
| Endothermic
| Flexible

#### Head  
| Property |
|:----------|
| Endothermic
| Lightning rod

}}


\page

### Gold  

:
**Material Type:** Metal  
**Rarity:** Uncommon  
**Tools:** [Smiths]   
**Difficulty:** DC: 11  
**Crafting Time:** 1.75 x Cost Hours

{{column-count:3

#### Handle  
| Property |
|:----------|
| Keen

#### Binding  
| Property |
|:----------|
| Flexible

#### Head  
| Property |
|:----------|
| Soft

}}

### Obsidian 

:
**Material Type:** Stone  
**Rarity:** Uncommon  
**Tools:** [Masons]    
**Difficulty:** DC: 13  
**Crafting Time:** 2.25 x Cost Hours

{{column-count:3

#### Handle  
| Property |
|:----------|
| Breakable 3
| Jagged Edge
| Dangerous

#### Binding  
| Property |
|:----------|
| Breakable 3
| Jagged Edge
| Dangerous

#### Head  
| Property |
|:----------|
| Breakable 3
| Jagged Edge
| Dangerous
| Sharpened


}}


### Silver  

:
**Material Type:** Metal  
**Rarity:** Uncommon  
**Tools:** [Smiths]   
**Difficulty:** DC: 12  
**Crafting Time:** 2 x Cost Hours

{{column-count:3

#### Handle  
| Property |
|:----------|
| Lightweight
| Magical

#### Binding  
| Property |
|:----------|
| Polished
| Magical

#### Head  
| Property |
|:----------|
| Magical

}}


\column

### Slime Core
The core itself isnt the part, rather the slime that forms around it. To do so, you must use magic to cause the core to reform in the correct shape of the part, rather than the slimes normal shape. Most of the material itself comes from the slime.
:
The damage done by Empowered core is the additional damage from the ooze. This is normally acid, but in cases such as the ooblex, its psychic.
:
**Material Type:** Monstrous  
**Rarity:** Uncommon  
**Tools:** [Alchemists]    
**Difficulty:** DC: 13  
**Crafting Time:** 2.25 x Cost Hours

{{column-count:3

#### Handle 
| Property |
|:----------|
| Empowered Core
| Flexible

\column

#### Binding  
| Property |
|:----------|
| Empowered Core
| Flexible

\column

#### Head
| Property |
|:----------|
| Shattered (core breaks)
| Empowered Core
| Flexible
| Engulf
| Blunt

}}




### Stale Bread
Its STILL bread. Its just been awhile! Its also noticeably harder.
:
**Material Type:** Organic  
**Rarity:** Common  
**Tools:** [Special (Wait 1 week with a bread part)]  
**Difficulty:** N/A  
**Crafting Time:** 7 days

{{column-count:3

#### Handle  
| Property |
|:----------|
| Lightweight	|
| Cheapskate | 

#### Binding  
| Property |
|:----------|
| Lightweight	|
| Cheapskate | 


#### Head  
| Properties
|:----------|
| Lightweight	|
| Blunt    |
| Cheapskate | 

}}




### Steel  
 
:
**Material Type:** Metal  
**Rarity:** Uncommon   
**Tools:** [Smiths]   
**Difficulty:** DC: 13  
**Crafting Time:** 2.25 x Cost Hours

{{column-count:3

#### Handle  
| Property |
|:----------|
| Heavy

#### Binding  
| Property |
|:----------|
| Reinforced

#### Head  
| Property |
|:----------|
| Reinforced

}}

\page

### Treant wood 

:
**Material Type:** Organic  
**Rarity:** Uncommon  
**Tools:** [Woodcarvers]    
**Difficulty:** DC: 10  
**Crafting Time:** 1.5 x Cost Hours

{{column-count:3

#### Handle  
| Property |
|:----------|
| Lightweight


#### Binding  
| Property |
|:----------|
| ---

#### Head  
| Property |
|:----------|
| Sentient


}}
\page

## Rare Materials

### High-grade Steel

:
**Material Type:** Metal  
**Rarity:** Rare  
**Tools:** [Smiths, Special (Forged in intense heat, than cooled slowly.)]   
**Difficulty:** DC: 15  
**Crafting Time:** 3.5 x Cost Hours

{{column-count:3

#### Handle  
| Property |
|:----------|
| Siege
| Reinforced
| Heavy

#### Binding  
| Property |
|:----------|
| Siege
| Reinforced
| Heavy

#### Head  
| Property |
|:----------|
| Siege
| Reinforced
| Heavy


}}
### Bled Celestial Feather
The feather of a slain celestial will always be worse than one that is given and pure, one gainned from a corpse or against its will is said to drive those who wield it insane. Even if its a feather, that doesnt mean its soft, being as sharp as steel.
:
**Material Type:** Planar  
**Rarity:** Rare  
**Tools:** [Weavers]   
**Difficulty:** DC: 14  
**Crafting Time:** 3.25 x Cost Hours

{{column-count:3

#### Handle  
| Property |
|:----------|
| Hollow
| Sterile
| Holy
| Lightweight
| Insanity

\column

#### Binding  
| Property |
|:----------|
| Lightweight
| Moon-touched
| Holy
| Insanity

\column

#### Head  
| Property |
|:----------|
| Sharpened
| Holy
| Planar
| Lightweight
| Insanity 2

}}

### Dragonbone 
Parts carved from the bones of dragons, still holding parts of their power within.
:
**Material Type:** Monstrous  
**Rarity:** Rare  
**Tools:** [Woodcarvers]  
**Difficulty:** DC: 14  
**Crafting Time:** 3.25 x Cost Hours

{{column-count:3

#### Handle  
| Property |
|:----------|
| Draconic Will
| Magical

#### Binding  
| Property |
|:----------|
| Draconic Will
| Magical


#### Head  
| Properties
|:----------|
| Draconic Will
| Empowered Core
| Magical

}}



\column

### Electrum  

:
**Material Type:** Metal  
**Rarity:** Rare  
**Tools:** [Smiths]   
**Difficulty:** DC: 12  
**Crafting Time:** 2.75 x Cost Hours

{{column-count:3

#### Handle  
| Property |
|:----------|
| Magical
| Lightning rod

#### Binding  
| Property |
|:----------|
| Shocking

#### Head  
| Property |
|:----------|
| Shocking
| Soft

}}




### Greater Construct

The part itself is a construct. It has become more capable overall, gainning more sentience. This can be made by making a golem/construct in the shape of the part, or by carving/reforging an existing construct thats CR 5--8.
:
If carved or reforged, it retains its mental stats and alignment. If crafted, roll its stats, and you choose its alignment.
:
**Material Type:** Monstrous OR Alchemical  
**Rarity:** Rare  
**Tools:** [Smiths/Masons/Woodcarvers (When crafted using an existing construct) OR Tinkers (Crafting)]    
**Difficulty:** DC: 14  
**Crafting Time:** 3.25 x Cost Hours

{{column-count:3

#### Handle  
| Property |
|:----------|
| Sentient
| Reinforced
| Keen

#### Binding  
| Property |
|:----------|
| Sentient
| Reinforced
| Magnetic

#### Head  
| Property |
|:----------|
| Sentient 2
| Reinforced
| Keen

}}




### Hardened Glass 

:
**Material Type:** Stone  
**Rarity:** Rare  
**Tools:** [Glassblowers]    
**Difficulty:** DC: 14  
**Crafting Time:** 3.25 x Cost Hours

{{column-count:3

#### Handle  
| Property |
|:----------|
| Breakable
| Jagged Edge
| Dangerous

#### Binding  
| Property |
|:----------|
| Breakable
| Jagged Edge
| Dangerous

#### Head  
| Property |
|:----------|
| Breakable
| Jagged Edge
| Dangerous
| Sharpened

}}



\page

### Ironwood
Wood thats been infused with iron, or a wood so dense and tough that it might as well be iron. both are correct, though its much harder to find natural ironwood, only growing in specific areas, or in the feywilds. 
:
compared to the challenge of making said ironwood yourself by infusing iron into wood without charing it. 
:
**Material Type:** Organic  
**Rarity:** Rare  
**Tools:** [Woodcarvers]   
**Difficulty:** DC: 15  
**Crafting Time:** 3.5 x Cost Hours

{{column-count:3

#### Handle  
| Property |
|:----------|
| Lightweight
| Reinforced

#### Binding  
| Property |
|:----------|
| Reinforced

#### Head  
| Property |
|:----------|
| Blunt
| Reinforced


}}

### Jewels 
 
:
**Material Type:** Stone  
**Rarity:** Rare  
**Tools:** [Jewelers]   
**Difficulty:** DC: 13  
**Crafting Time:** 3 x Cost Hours

{{column-count:3

#### Handle  
| Property |
|:----------|
| Magic Focus
| Slippery Grip

#### Binding  
| Property |
|:----------|
| Magic Focus
| Shattered

#### Head  
| Property |
|:----------|
| Magic Focus
| Jagged Edge
}}

### Platinum  
 
:
**Material Type:** Metal  
**Rarity:** Rare  
**Tools:** [Smiths]   
**Difficulty:** DC: 14  
**Crafting Time:** 3.25 x Cost Hours

{{column-count:3

#### Handle  
| Property |
|:----------|
| Sterile
| Magical

#### Binding  
| Property |
|:----------|
| Heat Retention
| Magical
| Shimmering

#### Head  
| Property |
|:----------|
| Heat Retention
| Holy
| Magical

}}


\column

### Quicksilver
Quicksilver is another name for mercery.
:
**Material Type:** Metal  
**Rarity:** Rare  
**Tools:** [Calligraphers (Freezing it into a solid)]   
**Difficulty:** DC: 14  
**Crafting Time:** 3.25 x Cost Hours

{{column-count:3

#### Handle  
| Property |
|:----------|
| Insanity
| Magic Focus
| Heavy

#### Binding  
| Property |
|:----------|
| Flexible
| Magical

#### Head  
| Property |
|:----------|
| Poisonous
| Withering

}}

### Sunsteel

:
**Material Type:** Metal  
**Rarity:** Rare  
**Tools:** [Smiths, Special (Forged under blazing desert sun.)]   
**Difficulty:** DC: 14  
**Crafting Time:** 3.25 x Cost Hours

{{column-count:3

#### Handle  
| Property |
|:----------|
| Shimmering
| Magical

#### Binding  
| Property |
|:----------|
| Shimmering
| Burning

#### Head  
| Property |
|:----------|
| Heat Retention
| Shimmering
| Magical

}}

### Troll Bone
 
:
**Material Type:** Monstrous  
**Rarity:** Rare  
**Tools:** [Woodcarvers]   
**Difficulty:** DC: 12  
**Crafting Time:** 2.75 x Cost Hours

{{column-count:3

#### Handle  
| Property |
|:----------|
| Hollow
| Heavy

\column

#### Binding  
| Property |
|:----------|
| Shimmering
| Leeching

\column

#### Head  
| Property |
|:----------|
| Spiked
| Leeching

}}


\page

## Very Rare Materials

### Adamantine  

:
**Material Type:** Metal  
**Rarity:** Very Rare  
**Tools:** [Smiths, Special (Extreme Heat)]   
**Difficulty:** DC: 17  
**Crafting Time:** 5.5 x Cost Hours

{{column-count:3

#### Handle  
| Property |
|:----------|
| Heat Retention
| Burning
| Magical
| Reinforced

#### Binding  
| Property |
|:----------|
| Heat Retention
| Burning
| Magical
| Shimmering

#### Head  
| Property |
|:----------|
| Heat Retention 2
| Burning 2
| Magical

}}

### Aerosite
Condensed storm-wind crystal, gathered from the eye of a hurricane or formed from slain Air Elementals.

:
**Material Type:** Planar (Elemental Air)  
**Rarity:** Very Rare  
**Tools:** [Calligraphers, Special (Formed in the eye of a storm)]   
**Difficulty:** DC: 17  
**Crafting Time:** 5.5 x Cost Hours

{{column-count:3

#### Handle  
| Property |
|:----------|
| Lightweight
| Shocking
| Magical

#### Binding  
| Property |
|:----------|
| Lightweight
| Shocking
| Magical

#### Head  
| Property |
|:----------|
| Lightweight
| Shocking 2
| Returning
| Magical

}}

### Aquacite
Formed from crystallized elemental water, found in the deepest ocean trenches or torn from a Water Elemental’s core.

:
**Material Type:** Planar (Elemental Water)  
**Rarity:** Very Rare  
**Tools:** [Jewelers, Special (Carved in the deep sea)]   
**Difficulty:** DC: 17  
**Crafting Time:** 5.5 x Cost Hours

{{column-count:3

#### Handle  
| Property |
|:----------|
| Flexible
| Chilling
| Magical

#### Binding  
| Property |
|:----------|
| Flexible
| Chilling
| Magical

#### Head  
| Property |
|:----------|
| Flexible
| Chilling 2
| Engulf
| Magical

}}


\column
### Celestial Feather
A feather gifted from a celestial, known for being one of the most pure things there is. Even if its a feather, that doesnt mean its soft, being as sharp as steel.
:
**Material Type:** Planar  
**Rarity:** Very Rare  
**Tools:** [Weavers]   
**Difficulty:** DC: 16  
**Crafting Time:** 5.25 x Cost Hours

{{column-count:3

#### Handle  
| Property |
|:----------|
| Hollow
| Sterile
| Holy
| Moon-touched 
| Lightweight


\column

#### Binding  
| Property |
|:----------|
| Lightweight
| Moon-touched 
| Holy


\column

#### Head  
| Property |
|:----------|
| Sharpened
| Holy 2
| Planar
| Moon-touched
| Lightweight


}}

### Dragonfire Glass

:
**Material Type:** Stone  
**Rarity:** Very Rare  
**Tools:** [Glassblowers, Special (Forged in dragon fire)]    
**Difficulty:** DC: 17  
**Crafting Time:** 5.5 x Cost Hours

{{column-count:3

#### Handle  
| Property |
|:----------|
| 
| Jagged Edge
| Dangerous
| Draconic Will

#### Binding  
| Property |
|:----------|
| 
| Jagged Edge
| Dangerous
| Draconic Will

#### Head  
| Property |
|:----------|
| 
| Jagged Edge 2
| Dangerous 2
| Sharpened
| Draconic Will

}}

### Elite Construct

The part itself is a construct. At this point, the part is getting strong enough, that it can't be crafted by the average person, and must be made by carving/reforging an existing construct thats CR 9--13.
:
it retains its mental stats and alignment.
:
**Material Type:** Monstrous  
**Rarity:** Very Rare  
**Tools:** [Smiths/Masons/Woodcarvers]    
**Difficulty:** DC: 16  
**Crafting Time:** 5.25 x Cost Hours

{{column-count:3

#### Handle  
| Property |
|:----------|
| Sentient 2
| Reinforced
| Keen
| Magic Resistant

#### Binding  
| Property |
|:----------|
| Sentient 2
| Reinforced
| Magnetic
| Magic Resistant

#### Head  
| Property |
|:----------|
| Sentient 3
| Reinforced
| Keen
| Magic Resistant

}}


\page
### Ignisite
Forged from cooled shards of pure elemental flame, harvested from the inside of an active volcano or from a Fire Elemental’s core.

:
**Material Type:** Planar (Elemental Fire)  
**Rarity:** Very Rare  
**Tools:** [Smiths, Special (Forged in a volcano)]   
**Difficulty:** DC: 17  
**Crafting Time:** 5.5 x Cost Hours

{{column-count:3

#### Handle  
| Property |
|:----------|
| Glowing
| Burning
| Magical

#### Binding  
| Property |
|:----------|
| Glowing
| Burning
| Magical

#### Head  
| Property |
|:----------|
| Glowing
| Burning 2
| Dangerous
| Magical

}}

### Lich
This isnt made of a liches bones, its made with a modified phylactery, turning the lich itself into the part. Due to this transformation, the lich cannot cast spells, though he is NOT happy about this. Maybe make sure his sentience isnt too high. The sentience in the first head overtakes any other sentience in the weapon, consuming them. The part only needs the phylactery, the rest of the material will regenerate on its own.

***I have no mouth, and i must scream - Some lich made into a Dagger***
:
**Material Type:** Monstrous  
**Rarity:** Very Rare  
**Tools:** [Calligraphers (Modifying the phylactery]   
**Difficulty:** DC: 18  
**Crafting Time:** 6 x Cost Hours

{{column-count:3

#### Handle  
| Property |
|:----------|
| Magical
| Insanity 2
| Hollow
| Dangerous
| Magical
| Sentient 3 (Keeps the liches mental stats, Alignment, and memories)
| Magic Focus

\column

#### Binding  
| Property |
|:----------|
| Magical
| Insanity 2
| Hollow
| Dangerous
| Magical
| Sentient 3 (Keeps the liches mental stats, Alignment, and memories)
| Magic Focus

\column

#### Head  
| Property |
|:----------|
| Magical
| Insanity 3
| Hollow
| Dangerous
| Magical
| Sentient 3 (Keeps the liches mental stats, Alignment, and memories)
| Magic Focus
| Withering

}}


\column

### Mithral  

:
**Material Type:** Metal  
**Rarity:** Very Rare  
**Tools:** [Smiths]   
**Difficulty:** DC: 16  
**Crafting Time:** 5.25 x Cost Hours

{{column-count:3

#### Handle  
| Property |
|:----------|
| Lightweight
| Magical
| Magic Resistant
| Glowing 

#### Binding  
| Property |
|:----------|
| Lightweight
| Magical
| Magic Resistant
| Glowing

#### Head  
| Property |
|:----------|
| Lightweight
| Magical
| Magic Resistant
| Moon-touched
| Glowing 2

}}


### Starmetal
:
**Material Type:** Metal/Planar  
**Rarity:** Very Rare  
**Tools:** [Smiths, Special (Forged under a falling star)]    
**Difficulty:** DC: 17  
**Crafting Time:** 5.5 x Cost Hours

{{column-count:3

#### Handle  
| Property |
|:----------|
| Magnetic
| Magic Resistant
| Magical
| Planar

#### Binding  
| Property |
|:----------|
| 
| Magnetic
| Magical
| Magic Resistant
| Planar

::
#### Head  
| Property |
|:----------|
| Planar 2
| Reinforced
| Moon-touched
| Magic Resistant
| Magical

}}






### Terracite
Dense mineral infused with elemental earth, quarried from the bottom of mountains or mined from slain Earth Elementals.

:
**Material Type:** Planar (Elemental Earth)  
**Rarity:** Very Rare  
**Tools:** [Masons, Special (Hammered in earthquake tremors)]   
**Difficulty:** DC: 17  
**Crafting Time:** 5.5 x Cost Hours

{{column-count:3

#### Handle  
| Property |
|:----------|
| Heavy
| Reinforced
| Magical

\column

#### Binding  
| Property |
|:----------|
| Heavy
| Reinforced
| Magical

\column

#### Head  
| Property |
|:----------|
| Heavy
| Blunt
| Shimmering
| Reinforced 2
| Magical

}}
\page

## Legendary Materials

### Eternal Glass
This glass is made from a material called "Starseed Sand" dropped from meteorites that fly through the air.
:
**Material Type:** Stone  
**Rarity:** Legendary  
**Tools:** [Calligraphers, Special (Forged under pure full moonlight in sacred grounds)]    
**Difficulty:** DC: 21  
**Crafting Time:** 9 x Cost Hours

{{column-count:3

#### Handle  
| Property |
|:----------|
| 
| Jagged Edge 2
| Dangerous
| Sterile
| Glowing
| Magical
| Moon-touched

#### Binding  
| Property |
|:----------|
| 
| Jagged Edge 2
| Dangerous
| Glowing
| Magical
| Moon-touched

\column
#### Head  
| Property |
|:----------|
| 
| Jagged Edge 2
| Dangerous
| Sharpened
| Glowing
| Magical
| Moon-touched 2
| Holy 2


}}

### Nyxium
This alloy is made with a combination of adamantine and starmetal, striking with the burning force of the sun and the chilling hunger of the void. It is pure black, with a white boarder, similar to a black hole.
:
**Material Type:** Metal/Planar  
**Rarity:** Legendary  
**Tools:** [Smiths, Special (Forged during a total eclipse)]   
**Difficulty:** DC: 21  
**Crafting Time:** 9 x Cost Hours

{{column-count:3

#### Handle  
| Property |
|:----------|
| Magic Resistant
| Withering
| Burning
| Magical
| Glowing -1 (produces darkness)
| Moon-touched -1 (produces darkness; still gains +1)
| Magic Focus


\column

#### Binding  
| Property |
|:----------|
| Withering
| Burning
| Shimmering 2
| Magical
| Magic Resistant
| Glowing -1 (produces darkness)
| Moon-touched -1 (produces darkness; still gains +1)


\column

#### Head  
| Property |
|:----------|
| Planar 2
| Withering 2
| Burning 2
| Magical
| Glowing -1 (produces darkness)
| Moon-touched -1 (produces darkness; still gains +1)

}}


\column

### Orichalchum 

:
**Material Type:** Metal  
**Rarity:** Legendary  
**Tools:** [Smiths]   
**Difficulty:** DC: 21  
**Crafting Time:** 9 x Cost Hours

{{column-count:3

#### Handle  
| Property |
|:----------|
| Sterile
| Sentient
| Magic Focus
| Magical
| Shimmering
| Magic Resistant
| Reinforced

#### Binding  
| Property |
|:----------|
| Sentient
| Magic Focus
| Magical
| Shimmering
| Polished
| Reinforced


#### Head  
| Property |
|:----------|
| Sentient 2
| Magic Focus
| Magical
| Shimmering 2
| Reinforced 2
| Polished

}}

### The Smith Family’s Indestructible Yuletide Fruitcake

"It is impossible to say who first devised the recipe for this thing, or what it was originally intended for, as it is most certainly not edible... Atleast, not without breaking every tooth in your mouth."
:
**Material Type:** Organic?  
**Rarity:** Legendary  
**Tools:** [Cooks]    
**Difficulty:** 20  
**Crafting Time:** 8.5 x Cost Hours

{{column-count:3

#### Handle  
| Property |
|:----------|
| Sticky
| Heavy
| Siege
| Reinforced
| Dangerous 2
| Heat Retention 2
| Wild Positive 3

#### Binding  
| Property |
|:----------|
| Keen
| Heavy
| Siege
| Reinforced
| Dangerous 2
| Heat Retention 2
| Wild Positive 3

#### Head  
| Property |
|:----------|
| Blunt
| Heavy
| Siege
| Reinforced
| Dangerous 2
| Heat Retention 2
| Wild Positive 3


}}

\page

### Ultimate Construct

The part itself is a construct. It has reached the point of being the best form of construct it could be. It must be made by carving/reforging an existing construct thats CR 14 or higher.
:
it retains its mental stats and alignment.
:
**Material Type:** Monstrous  
**Rarity:** Legendary  
**Tools:** [Smiths/Masons/Woodcarvers]    
**Difficulty:** 19  
**Crafting Time:** 8.25 x Cost Hours

{{column-count:3

#### Handle  
| Property |
|:----------|
| Sentient 2
| Reinforced
| Keen 2
| Magic Resistant

#### Binding  
| Property |
|:----------|
| Sentient 2
| Reinforced
| Magnetic 2
| Magic Resistant

#### Head  
| Property |
|:----------|
| Sentient 3
| Reinforced 2
| Keen 2
| Magic Resistant

}}


\page

## Material Tier Rules

These are the basic rules for making a material, granted these rules can be bent every now and again, it should rarely be done for balence reasons.
:
If you make a meterial, or have additional rules or changes and think it should be in the book itself, send it to me for review! (Discord: silvercrow605 Reddit: u/silvercrow605) if i think that you either gave enough materials, or give great insight for balence/rules, your free to make an example weapon to be added to the book, as well as art for it if you wish to provide your own! (no ai, as i post this in places that don't allow it, and proper credit must be given if you didnt make it.)

### Difficulty
The difficulty of crafting the parts depends on the rarity and the difficulty of working with the material. Each rarity has its own flat DC, with the difficulty added onto that.

##### Rarity & Difficulty
{{column-count:2
| Rarity | DC |
|:----------------|:--:|
| Common          | 11 |
| Uncommon        | 12 |
| Rare            | 13 |
| Very Rare       | 15 |
| Legendary       | 18 |

| Difficulty   | Modifier |
|:-------------|:--------:|
| Very Easy    |    -2    |
| Easy         |    -1    |
| Normal       |    +0    |
| Hard         |    +1    |
| Very Hard    |    +2    |
| Hell         |    +3    |
}}

### Crafting Time
The time it takes to make a weapon depends on its difficulty, and rarity, similarly to the Difficulty section before, having its own flat DC, and modifier, in addition to being multiplied by the material count to determine the amount of hours.

##### Rarity & Difficulty
{{column-count:2
| Rarity | DC |
|:----------------|:--:|
| Common          | 1 |
| Uncommon        | 2 |
| Rare            | 3 |
| Very Rare       | 5 |
| Legendary       | 8 |

| Difficulty   | Modifier |
|:-------------|:--------:|
| Very Easy    |    -0.5  |
| Easy         |    -0.25 |
| Normal       |    +0    |
| Hard         |    +0.25 |
| Very Hard    |    +0.5  |
| Hell         |    +1    |
}}


\column

### Common Materials

- **Property Limits:** Positive properties max out at 1. Negative properties can go higher for all tiers (for example, Breakable 5).
:
- **Number of Properties:** 1–2 per part, often one positive with an optional negative.
:
- **Examples:** Iron, Wood, Lead, Glass, Bone.

### Uncommon Materials

- **Property Limits:** Positive properties usually cap at 1, sometimes 2 if there are negatives.
:
- **Number of Properties:** 2–3 per part.
:
- **Examples:** Steel, Silver, Gold, Obsidian, Treant Wood.

### Rare Materials

- **Property Limits:** Positive properties can reach 2.
:
- **Number of Properties:** 2–3 per part.
:
- **Examples:** Dragonbone, Platinum, Troll Bone, Hardened Glass.



### Very Rare Materials

- **Property Limits:** Positive properties reach 3, normally having 1-2 tier 2, extremely rarely tier 3.
:
- **Number of Properties:** 3–4 per part.
:
- **Examples:** Adamantine, Mithral, Dragonfire Glass, Elite Construct.
:
### Legendary Materials

- **Property Limits:** Positive properties can reach 3, normally having 3 tier 2, rarely tier 3. Negative properties are rare.
:
- **Number of Properties:** 5+ per part, normally stacking multiple high-level effects.
:
- **Examples:** Orichalcum, Eternal Glass, Ultimate Construct.

\page
## Weapon examples
I asked some friends to make items using the materials, here are some they made as examples of weapons. I also allowed people who give to this book the ability to add one as well.


### The Good Orc's Will
*Melee weapon, Greatclub, Tough Rod (Orichalchum)*
:
Two-Handed, 1d8 Bludgeoning Damage
:
Sentient 2 (Int 8, Wis 11, Cha 13 Lawful good, Speaks, reads, and understands Orc, Hearing and normal vision out to 60 feet.)
:
Magic Focus, Magical, Shimmering 2, Reinforced 2, Polished

{{artist,top:200px,right:150px
##### Original By:
Polar Engine
}}



![Good Orc's Will](https://i.imgur.com/8sSxqXF.png) {position:absolute,top:50px,right:60px,width:360px}
:::::::

### Glass Cannon
*Melee weapon, Handaxe, Tool Rod (Glass), Binding (Glass), Axe Head (Glass)*
:
Light, thrown (range 20/60), 4d6 Slashing
:
Breakable 5 (All parts break upon use)
:
Jagged Edge 3, Dangerous 3, Sharpened



![Glass Cannon](https://i.imgur.com/rPijtXQ.png) {position:absolute,top:350px,right:200px,width:170px}

{{artist,top:450px,right:140px
##### Original By:
Starlight Sorcerer
}}
:::::::::
### Thor's Backup
*Melee weapon, Greataxe,Tough Rod (Brass), Tough Binding (Bronze), Large Axe Head (Stone), Axe Head (Electrum)*
:
*This weapon was made by the person rolling parts selectively at random.*
:
Heavy, two-handed, 1d12 Bludgeoning + 1 lightning
:
As a reaction, you gain resistance to lightning damage, and gain an additional 1d6 lightning on your next attack. This can be done once per long rest.
:
Sterile, Reinforced, Blunt, Shocking, Lightning rod
:
**THIS ITEM IS OUT OF DATE. HAS TO BE UPDATED**

![Thor's Backup](https://i.imgur.com/DObIRAE.png) {position:absolute,top:600px,right:100px,width:370px}

{{artist,top:850px,right:170px
##### Original Parts By:
Reagan Wigboldy

Loot Tavern

bradlyvancamp
::::
or just use an image of Stormbreaker, idk lol
}}


\page
### Twilight Harvest
*Melee weapon, Glaive, Tough Rod (Dragonbone), Tough Rod (Dragonbone), Sword Blade (Jewels [Amethyst])*
:
*This weapon was suggested by Dracolich for his contributions.*
:
Heavy, reach, two-handed, 1d10 slashing Damage
:
Draconic Will 2, Magical, Magic Focus, Jagged Edge
:


{{artist,top:200px,right:150px
##### Original Parts By:
Joseph Bentley

Ahlvie
}}

![Twilight Harvest](https://i.imgur.com/ZfLsPOp.png) {position:absolute,top:0px,right:-10px,width:500px}
:::::::

### Malaria
*Melee weapon, Rapier, Sword Blade (Troll Bone), Crossbar (Troll Bone), Tool Rod (Troll Bone Handle)*
:
*This weapon was suggested by Milo for his contributions.*
:
Heavy, Finesse, 1d8 piercing
:
Upon hitting a target, you can regain half of the weapons Piercing damage as hit points. This can be done twice per long rest.
:
Hollow, Heavy, Shimmering, Leeching 2, Spiked
:

{{artist,top:400px,right:150px
##### Original By:
boomboompowboyz

}}



![Malaria](https://i.imgur.com/QtuZh6r.png) {position:absolute,top:370px,right:-110px,width:600px}
:::::::

### Earthquake
*Melee weapon, Flail, Plate (Terracite), String (Gold), Tool Rod (Silver)*
:
*This weapon was suggested by Vampiric12 for his contributions.*
:
1d8 bludgeoning damage
:
Can reroll a 1 on a damage roll twice per round.
:
Blunt, Shimmering, Reinforced 2, Magical, (Lightweight cancels Heavy)

{{artist,top:790px,right:260px
##### Original From:
D&D Player’s Handbook (2024)

}}



![Earthquake](https://i.imgur.com/RZ6Kzs1.png) {position:absolute,top:650px,right:50px,width:350px}
:::::::

\page
### Reach The Sun
*Melee weapon, Greatsword, Large Blade (High-grade Steel), Tough Rod (Sunsteel), Tough Rod (Gallium)*
:
*This weapon was suggested by Background-Record685 for his contributions.*
:
Heavy, Two-Handed, 2d6 slashing + 1 fire damage
:
The weapon deals double damage to objects and structures.
:
+5 reach when dealing fire damage, +2 bludgeoning damage when dealing cold damage.
:
Once per round, you can reroll a 1 on a damage roll.
:
Disadvantage on attack rolls in the rain, or while bloodied.
:
Siege, Reinforced, Heavy, Shimmering, Burning, Endothermic, Flexible, Slippery Grip
:


{{artist,top:200px,right:260px
##### Original By:

kimkkyu_
}}

![Reach The Sun](https://i.imgur.com/GxmzVz3.png) {position:absolute,top:-40px,right:-10px,width:480px}
:::::::

\page

## Changelog

- 0.3:

  - Initial Release.

- 0.4: 

  - Added materials: Treant Wood, Construct tier for each level, Slime Core, & Troll Bone.

  - Added properties: Engulf.
  
  - Added examples: Image for "Thor's Backup."
  
  - Added Gauntlets to the list of recipes. counting as another unarmed weapon.
  
  - Moved Parts by Color up to page 3.
  
- 0.4.5:
  
    - Added materials: Obsidian, Dragonfire Glass, & Eternal Glass.
    
    - Changed crafting recipe for Gauntlets.
    
- 0.5:
  
  - Added Material making rules, and my contact locations for sharing ideas.
  
- 0.5.5:

  - Fixed the rarities being listed wrong on the construct materials.
  
  - Fixed the grammar and spelling mistakes.
  
  - Changed the 10 attacks rules on twist shift, and vibrating edge to Roll 2d20, if both numbers match, you make their saves.
  
  - Added new contribuition example "Twilight Harvest."
  
- 0.6:

  - Added materials: Scrap.
  
  - Added properties: Wild Positive, Wild Negative.

  - Changed the DC to create a part to be higher at later rarities.
  
  - Added the total amount of material needed to make a weapon.
  
  - Alphabetized things.

- 0.7:

  - Added materials: Bread, Stale bread.

  - Moved Difficulty chart to "Material Tier Rules."
  
  - Replaced the difficulty names on the materials with their DCs.
  
  - Changed Crafting Tags to Tools Required.
  
  - Renamed Tool Parts, Tool Properties, and the word "tool" from material descriptions to avoid confusion with artisan's tools.
  
  - Added Crafting times.
  
- 0.7.3:

  - Added materials: Bamboo, Frostwood, Sunsteel, Starmetal, Nyxium, Quicksilver.
  
  - Added property: Insanity.
  
  - Sentient weapons at level 6 can now control the weapon itself.

- 0.7.5:

  - Readded the missing holy property.
  
  - Added materials: Elemental materials (Ignisite, Aquacite, Terracite, Aerosite.)
  
- 0.8:

  - Specified the rules clearer for the crafting process.
  
  - Added examples: Malaria, Earthquake.
  
- 0.8.5:

  - Added materials: ironwood, Bled Celestial Feather, Celestial Feather, Gallium, Air, Lich, Black iron.
  
  - Added property: Endothermic
  
  - Added optional rule for artificer abilities and the like.
  
  - Changed Arm wraps to be a binding instead of a head part.
  
- 0.9:

  - Added longer and shorter properties.
  
  - added cheapskate to brass head.
  
  - Swapped golds handle and head properties.
  
  - Black iron renamed to high-grade steel.
  
  - Swapped lightning rod and soft on electrums handle and head.

- 0.9.1:
  
  - Added example: Reach The Sun
  
- 0.9.2:

  - Removed cheapskate from wood handle.
  - Revised the lightweight & heavy properties to cancel eachother out better.
  
- 0.9.3:
  - Added materials: The Smith Family’s Indestructible Yuletide Fruitcake
10:16