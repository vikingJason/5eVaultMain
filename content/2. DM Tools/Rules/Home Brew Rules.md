...
With this option, a creature uses the special [[content/0. DM Screen/Rules/PF2E/traits/shove]] attack from the "Player's Handbook" to force a target to the side, rather than away. The attacker has disadvantage on its Strength ([[Athletics]]) check when it does so. If that check is successful, the attacker moves the target 5 feet to a different space within its reach.
